...
# данные по холоду
tab_cold <- read.xlsx("F:/Proccessing/Results/tab_svod_region_cold_peak_95.xlsx")
tab_cold <- tab_cold %>% filter(!Region_name_ROSBKS %in% c("Чеченская Республика", "Ингушская республика", "Республика Дагестан"))

tab_hot <- read.xlsx("F:/Proccessing/Results/tab_svod_region_hot_peak_95.xlsx")
tab_hot <- tab_hot %>% filter(!Region_name_ROSBKS %in% c("Чеченская Республика", "Ингушская республика", "Республика Дагестан"))



# читаем основу карты
osnov <- st_read("F:/Proccessing/Data/rus_osnova/rus_lvl2.shp")
osnov <- ms_simplify(osnov, keep = 0.05) # значение keep отражает долю оставляемых углов полигонов. 0.05 == 5%, 0.005 == 0,5%

osnov$NAME <- str_replace_all(osnov$NAME, pattern = c("Кемеровская область" = "Кемеровская область — Кузбасс",
                                                      "город федерального значения Санкт-Петербург" = "город Санкт-Петербург",
                                                      "город федерального значения Москва" = "город Москва",
...
obsh <-
  ...
  scale_fill_gradientn(colours =  c("white", "grey5"), name = "ото всех\nволн,\n% ото всех\nсмертей", na.value = "khaki", limits = c(0, 2.2)) +
...
tab_cold_risk <- read.xlsx("F:/Proccessing/Results/tab_svod_region_cold_95_risk.xlsx")
tab_cold_risk <- tab_cold_risk %>% filter(!Region_name_ROSBKS %in% c("Чеченская Республика", "Ингушская республика", "Республика Дагестан"))

tab_hot_risk <- read.xlsx("F:/Proccessing/Results/tab_svod_region_hot_95_risk.xlsx")
...
# читаем основку
osnov <- st_read("F:/Proccessing/Data/rus_osnova/rus_lvl2.shp")
osnov <- ms_simplify(osnov, keep = 0.05)

osnov$NAME <- str_replace_all(osnov$NAME, pattern = c("Кемеровская область" = "Кемеровская область — Кузбасс", 
                                                      "город федерального значения Санкт-Петербург" = "город Санкт-Петербург", 
                                                      "город федерального значения Москва" = "город Москва", "республика" = "Республика", 
                                                      "Республика Чувашия" = "Чувашская Республика"))
...
cold_raz <-
  ...
  scale_fill_gradient2(low = "green", mid = "white", high = "darkblue", midpoint = 0,  limits = c(-0.3, 1.5), name = "Разница\nоценок\nдля волн\nжары, п.п.",  na.value = "khaki") + 
