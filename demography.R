...
output_waves_hot <- read.xlsx("F:/Proccessing/Results/output_waves_hot_95per.xlsx", detectDates = T)
output_waves_cold <- read.xlsx("F:/Proccessing/Results/output_waves_cold_95per.xlsx", detectDates = T)
...
tab_rosbks_raw <- read.csv("F:/Proccessing/Data/raw_data/rosbks_data_raw.csv")

# загружаем файл с назаниями регионов для РБКС
tab_rosbks_names <- read.xlsx("F:/Proccessing/Data/raw_data/rosbks_names.xlsx")
...
tab_region_population <- read_xls("F:/Proccessing/Data/raw_data/region_average_population.xls")
...
write.xlsx(tab_svod_region_cold,"F:/Proccessing/Results/tab_svod_region_cold.xlsx")
# write.xlsx(tab_waves_cold_regions_years,"F:/Proccessing/Results/waves_cold_regions_years_95.xlsx")

write.xlsx(tab_svod_region_hot,"F:/Proccessing/Results/tab_svod_region_hot.xlsx")
# write.xlsx(tab_waves_hot_regions_years,"F:/Proccessing/Results/waves_hot_regions_years_95.xlsx")
...
for (r in c(1 : length(code_region))) {
  ...
  ggsave(filename = paste0("F:/Proccessing/Results/hot_waves_95_week/", name_region[r], " волны жары.jpeg"), plot = grid, width = 526.8, height = 295.8, units = "mm", dpi = 300, limitsize = FALSE)
...
for (r in c(1 : length(code_region))) {
  ...
  # сохраняем как рисунок в папку
  ggsave(filename = paste0("F:/Proccessing/Results/cold_waves_95_week/", name_region[r], " волны холода.jpeg"), plot = grid, width = 526.8, height = 295.8, units = "mm", dpi = 300, limitsize = FALSE)
...
write.xlsx(x = tab_svod_region_hot_peak, file = "F:/Proccessing/Results/tab_svod_region_hot_peak_95.xlsx")
...
# сохраняем результирующую таблицу
write.xlsx(x = tab_svod_region_cold_peak, file = "F:/Proccessing/Results/tab_svod_region_cold_peak_95.xlsx")
...
# tab_meteo_all <- read.csv("F:/Proccessing/Data/raw_data/meteo_all.csv", sep = ";")

# list_station_tab <- openxlsx::read.xlsx("F:/Proccessing/Data/raw_data/meteostations_list.xlsx")
...
# аггрегация данных по каждой метеостанции по неделям
tab_met_week <- tab_met_st %>% group_by(week, year, ROSBKS_code) %>% 
  summarise(tmean = mean(TMEAN, na.rm = T), 
            ...
            name = first(Name), 
            height = first(Height), 
            ROSBKS_code = first(ROSBKS_code), 
            Region_name_ROSBKS = first(Region_name_ROSBKS))
...
# привязка демографических данных к метеоданным
...
for (z in c(1:70)) {
  ...
  cbtemp <- crossbasis(ewe_reg$tmean, 
                       lag = 0,
...
#   в зависимости от длины лага tmean устанавливается tmean = model$data$tmean[-c(1:4)]), где 4 - размер лага
...
tab_waves_cold_select <- left_join(x = tab_waves_cold_select, y = tab_rr_all, by = c("gradus" = "gradus", 
                                                                                     "Region_name_ROSBKS" = "reg"))
...
# сохраняем результирующую таблицу
write.xlsx(x = tab_svod_region_cold_risk, file = "F:/Proccessing/Results/tab_svod_region_cold_95_risk.xlsx")
...
# сохраняем результирующую таблицу
write.xlsx(x = tab_svod_region_hot_risk, file = "F:/Proccessing/Results/tab_svod_region_hot_95_risk.xlsx")
